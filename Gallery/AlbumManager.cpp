...
void AlbumManager::addPictureToAlbum()
{
	...
	Picture picture(0, picName); // the id doesn't matter 
...
void AlbumManager::showPicture()
{
	...
	std::cout << "1. Open with Paint" << std::endl;
	std::cout << "2. Open with default application" << std::endl;
	...
	if (choice == 1) {
		...
		std::string cmd = "\"C:\\Program Files\\IrfanView\\i_view64.exe\" \"" + pic.getPath() + "\"";
		launchImageProccess(cmd);
	}
}

void AlbumManager::launchImageProccess(std::string cmd)
{
	...
	if (CreateProcessA(NULL, const_cast<LPSTR>(cmd.c_str()), NULL, NULL, FALSE, 0, NULL, NULL, &info, &processInfo))
	{
		while (true) {
			if (WaitForSingleObject(processInfo.hProcess, 0) == WAIT_OBJECT_0) {
				// Process has terminated in some other way
				break;
			}
...
void AlbumManager::addUser()
{
	...
	User user(0,name); // the id doesn't metter
...
void AlbumManager::userStatistics()
{
	...
	std::cout << "user @" << userId << " Statistics:" << std::endl << "--------------------" << std::endl <<
		"  + Number of albums: " << m_dataAccess.countAlbumsOwnedOfUser(user) << std::endl <<
